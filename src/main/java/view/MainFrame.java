...
public final class MainFrame extends JFrame {
  ...
  private void jbOkActionPerformed(ActionEvent evt) {
    if (!this.jrbAnalise.isSelected() && !this.jrbBoletos.isSelected()) {
      JOptionPane.showMessageDialog(null, "Favor selecionar uma opdo tipo de procedimento", "Error", 0);
    } else {
      this.aux = 3;
  ...
  public static void main(String[] args) {
    ...
    EventQueue.invokeLater(new Runnable() {
          public void run() {
            try {
              UIManager.setLookAndFeel((LookAndFeel)new SyntheticaPlainLookAndFeel());
